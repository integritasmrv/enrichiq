#!/usr/bin/env python3
import subprocess
r = subprocess.run(['cat', '/tmp/api_main_backup.py'], capture_output=True, text=True)
content = r.stdout
s = content.find('@app.post("/webhook/hubspot")')
e = content.find('\nasync def query_rag')
before = content[:s]
after = content[e:]
lines = ['@app.post("/webhook/hubspot")',
...
'                    if ex:',
'                        sqla = "UPDATE nb_crm_customers SET enrichment_status = \\'To Be Enriched\\', \\"updatedAt\\" = NOW() WHERE hubspot_id = $1"',
'                        await conn.execute(sqla, hs_id)',
'                        results.append({"action": "updated", "crm": crm_name, "type": "company", "id": ex["id"]})',
'                    else:',
'                        sqli = "INSERT INTO nb_crm_customers (name, hubspot_id, enrichment_status, \\"createdAt\\", \\"updatedAt\\") VALUES ($1, $2, \\'To Be Enriched\\', NOW(), NOW()) RETURNING id"',
'                        row = await conn.fetchrow(sqli, name, hs_id)',
...
'                    if ex:',
'                        sqla = "UPDATE nb_crm_contacts SET enrichment_status = \\'To Be Enriched\\', \\"updatedAt\\" = NOW() WHERE hubspot_id = $1"',
'                        await conn.execute(sqla, hs_id)',
'                        results.append({"action": "updated", "crm": crm_name, "type": "contact", "id": ex["id"]})',
'                    else:',
'                        sqli = "INSERT INTO nb_crm_contacts (name, email, hubspot_id, enrichment_status, \\"createdAt\\", \\"updatedAt\\") VALUES ($1, $2, $3, \\'To Be Enriched\\', NOW(), NOW()) RETURNING id"',
'                        row = await conn.fetchrow(sqli, full, email or None, hs_id)',
