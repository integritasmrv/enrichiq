        ...
        try:
            async with httpx.AsyncClient(timeout=30.0, verify=False) as client:
                r = await client.post(
                    HATCHET_REST_URL,
                    headers={"Authorization": f"Bearer {HATCHET_TOKEN}", "Content-Type": "application/json"},
                    json={"key": "hubspot-sync", "payload": {"email": email, "firstname": firstname, "lastname": lastname, "company": company, "phone": phone}}
                )
                if r.status_code in (200, 201, 202):
                    print(f"[HATCHET] Pushed hubspot-sync via REST: {business_key}")
                    return {"status": "accepted", "workflow_id": f"hubspot-{business_key}", "engine": "hatchet"}
                else:
                    print(f"[HATCHET] Error: {r.status_code}")
                    return {"status": "error", "detail": f"Hatchet error: {r.status_code}"}
        except Exception as he:
            print(f"[HATCHET] REST error: {he}")
            return {"status": "error", "detail": f"Hatchet REST error: {str(he)[:100]}"}
