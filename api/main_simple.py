...
async def get_llm_response(prompt: str) -> str:
    ...
    if any(k in q for k in ['garantie','warranty','jaar']):
        return "Onze Energywall G1 komt met 10 jaar garantie! 🔒 Dat is de beste in de industrie."
    if any(k in q for k in ['cyclus','cycli','cycle']):
        return "De Energywall G1 gaat tot 50000 laadcycli mee! 🔋 Dat is 10x meer dan lithium batterijen."
    if any(k in q for k in ['hallo','hi','hello','hoi','hey','goede']):
        return "Hallo! 👋 Welkom bij Belinus! Waarmee kan ik je helpen?"
    return "Hallo! 👋 Ik help je graag met vragen over onze lithium-vrije batterijsystemen. Wat wil je weten?"
...
async def chatwoot_webhook(request: Request):
    try:
        ...
        prompt = f"""Context: {rag_context if rag_context else 'Geen specifieke context'}
Vraag: {user_message}
