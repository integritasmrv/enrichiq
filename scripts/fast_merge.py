...
class FastMerge:
    ...
    def mg(self, vid, sdt, vdn, sfl):
        ...
        self.cm()
        vd = self.cv(vdn)
        for t, p in [("enterprise","EnterpriseNumber"), ("establishment","EstablishmentNumber"), ("denomination","EntityNumber"), ("address",["entitynumber","typeofaddress"]), ("contact","EntityNumber"), ("activity",["entitynumber","activitygroup","naceversion","nacecode"]), ("branch","Id"), ("code","EntityNumber")]:
            self._mt(t, p, vd)
        self.m.commit()
        self.m.close()
        vd.close()

    def _mt(self, t, pk, vd, batch=5000):
        cs = vd.cursor()
        cd = self.m.cursor()
        s = "kbo." + t
        d = "kbo_master." + t
        ...
        cl = ", ".join(cols)
        ph = ", ".join(["%s"] * len(cols))
        sql = "INSERT INTO " + d + " (" + cl + ") VALUES (" + ph + ") ON CONFLICT (" + pk_str + ") DO UPDATE SET " + us
        tot = off = 0
        while True:
            cs.execute("SELECT * FROM " + s + " ORDER BY " + pk_str + " LIMIT " + str(batch) + " OFFSET " + str(off))
            rows = cs.fetchall()
            if not rows: break
            try:
